...
public class RoleManager {
     ...
     * @param roleNames Collection of role names (e.g., "admin", "seller", "user")
     */
    public void assignRolesToUser(User user, Collection<String> roleNames) {
    ...
    /**
     * Check if the current authenticated user has a specific role and is active
     * @param roleName The name of the role to check (without the "ROLE_" prefix)
     * @return true if the user has the role and is active, false otherwise
     */
    public boolean currentUserHasRole(String roleName) {
        ...
        if (authentication.getPrincipal() instanceof UserDetailsImpl) {
            ...
            if (!userDetails.isEnabled()) {
                logger.warn("User {} has role {} but is not active", userDetails.getUsername(), roleName);
                return false;
            }
        }
    ...
    /**
     * Check if the current authenticated user has any of the specified roles and is active
     * @param roleNames The names of the roles to check (without the "ROLE_" prefix)
     * @return true if the user has any of the roles and is active, false otherwise
     */
    public boolean currentUserHasAnyRole(String... roleNames) {
        ...
        if (authentication.getPrincipal() instanceof UserDetailsImpl) {
            UserDetailsImpl userDetails = (UserDetailsImpl) authentication.getPrincipal();
            if (!userDetails.isEnabled()) {
                logger.warn("User {} has one of the requested roles but is not active", userDetails.getUsername());
                return false;
            }
        }
    ...
    /**
     * Get the current authenticated user's roles as a collection of role names
     * @return Collection of role names (e.g., "ROLE_USER", "ROLE_SELLER")
     */
    ...
    /**
     * Convert a set of Role entities to a collection of role names
     * @param roles Set of Role entities
     * @return Collection of role names (e.g., "ROLE_USER", "ROLE_SELLER")
     */
    public Collection<String> convertRolesToRoleNames(Set<Role> roles) {
        return roles.stream()
    ...
    /**
     * Check if a user has a specific role and is active
    ...
    public boolean userHasRole(User user, String roleName) {
        ...
        // Check if user is active
        if (!user.isActive()) {
            logger.warn("User {} has role {} but is not active", user.getUsername(), roleName);
            return false;
        }
    ...
    /**
     * Check if a user has a specific role (ignoring active status)
     * @param user The user to check
     * @param roleName The name of the role to check (without the "ROLE_" prefix)
     * @return true if the user has the role, false otherwise
     */
    public boolean userHasRoleIgnoreStatus(User user, String roleName) {
        if (user == null || user.getRoles() == null || user.getRoles().isEmpty()) {
            return false;
        }
